...
interface ticketDao {
    ...
    fun getTicketsByFechas(fechaIni: LocalDate, fechaFin: LocalDate): Flow<List<TipoVentaTicket>>

    @Transaction
    @Query("select * from ticket WHERE estado == true")
    fun getTickets(): Flow<List<TipoVentaTicket>>
    ...
    @Transaction
    @Query(
        """SELECT * FROM ticket WHERE id_ticket LIKE '%'|| :any || '%'  
